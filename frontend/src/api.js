...
export const api = {
  ...
  customers: {
    ...
    merge: (targetId, sourceId) => post(`/customers/${targetId}/merge`, { sourceId }),
  ...
  dataManager: {
    ...
    import: (file, dataCategory, datasetName, assignTo) => { const fd = new FormData(); fd.append("file", file); fd.append("dataCategory", dataCategory); if (datasetName) fd.append("datasetName", datasetName); if (assignTo) fd.append("assignTo", assignTo); return postForm("/data-manager/import", fd); },
    ...
    // Bulk email campaigns (routes/emailCampaigns.routes.js) — server-side sending at random intervals.
    campaigns: () => get("/data-manager/email-campaigns"),
    campaign: (id) => get(`/data-manager/email-campaigns/${id}`),
    campaignRecipients: (id) => get(`/data-manager/email-campaigns/${id}/recipients`),
    createCampaign: (formData) => postForm("/data-manager/email-campaigns", formData),
    updateCampaign: (id, payload) => patch(`/data-manager/email-campaigns/${id}`, payload),
    startCampaign: (id) => post(`/data-manager/email-campaigns/${id}/start`),
    pauseCampaign: (id) => post(`/data-manager/email-campaigns/${id}/pause`),
    retryFailedCampaign: (id) => post(`/data-manager/email-campaigns/${id}/retry-failed`),
