...
export function useApiStore(enabled) {
  ...
  const dispatch = useCallback(async (action) => {
    switch (action.type) {
      ...
      case "CONVERT_LEAD_TO_DEAL": { const r = await api.leads.convertToDeal(action.id); await refresh(["leads", "deals"]); return r; }
      ...
      case "CHANGE_JOB_CUSTOMER": await api.jobCards.changeCustomer(action.id, action.customerId); return refresh(["jobCards"]);
